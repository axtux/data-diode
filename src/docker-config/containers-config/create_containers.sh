...
cd ../containers/diode-in/
sudo docker build . -t diode_in-img
sudo docker run -t -d --net intra-diode --ip 192.168.0.2 --name diode-in diode_in-img
cd -

# Building container : outter side of the diode
cd ../containers/diode-out/
sudo docker build . -t diode_out-img 
sudo docker run -t -d --net intra-diode --ip 192.168.0.3 --name diode-out diode_out-img
cd -

# Building container : machine on the outside network. FTP client, stream client. 
cd ../containers/outside-client/
