...
#sudo docker run -t -d --net intra-diode --ip 192.168.0.2 --name diode-in --cap-add=NET_ADMIN --cap-add=NET_RAW diode_in-img
sudo docker run -t -d --net secure-network --ip 10.0.0.2 -p 8880:80 --name diode-in --cap-add=NET_ADMIN --cap-add=NET_RAW diode_in-img
#sudo docker run -t -d --net secure-network -p 10.0.0.2:8880:80 --name diode-in --cap-add=NET_ADMIN --cap-add=NET_RAW diode_in-img
cd -
