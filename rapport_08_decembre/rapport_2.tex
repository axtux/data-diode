...
\item The database inside the secure network
\end{itemize}
...
\subsubsection{Risk 13 : Malware inside the secure side}
\textbf{Source} \\The secure network is infected by a malware.\\
\textbf{Component} \\The inside machine.\\
\textbf{Description}\\This malware could flood the inside machine with http requests.   \\
\textbf{Impact}\\
The flood of http request to the inside machine can create a several tens of minutes downtime.\\
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Confidentiality} & \textbf{Integrity} & \textbf{Availability} \\
...
\hline
13 & Malware in the secure network & Major & Unlikely & Medium \\
\hline
...
We can mitigate this risk by limiting the amount of data that a user can send in a define time.
\subsubsection{Risk 5 : Flooding of the outside webpage}
We can mitigate this risk by temporary blocking the opponent's IP adress when whe detect that the amount of failed connection attemps is too big to just be an administrator mistake.
...
\subsubsection{Risk 11 : Deletion of administrator accounts}
As the behaviour and the good will of the employees are under the responsability of the client, we transfer this risk to them.
\subsubsection{Risk 12 : CSRF on outside webpage}
...
\subsection{Sender}
We can \textit{sender} the server of the data-diode that is facing the outside network. It's mission is to push data to the other server of the data-diode, called the \textit{receiver}.

\subsection{Receiver}

The technical details go as follows :
\begin{itemize}
	\item{\textbf{OS} : Each server runs a 64 bits \textit{ubuntu server} 16.04  distribution}
	\item{\textbf{Software}: Both servers run an Nginx webserver}
\end{itemize}
